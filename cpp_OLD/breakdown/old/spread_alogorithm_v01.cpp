...
struct estim_data {
    ...
    std::string desc;
    double cost;
    spread_basis sb;     // structure of various helper fields define cost types ( Labor, Material, Discipline, Company Division-Cost Center)
};

struct apply_spreads {
    int row_id;
...
void populate_spread_true_false_for_records(std::vector<estim_data>& ed, std::vector<spread_costs_items>& sprds, std::vector<apply_spreads> &app_sprds) ;
void compute_spread_amounts(std::vector<estim_data>& ed, std::vector<spread_costs_items>& sprds, std::vector<apply_spreads> &app_sprds);
void print_estim_data_w_spreads(std::vector<estim_data>& ed, std::vector<spread_costs_items>& sprds, std::vector<apply_spreads> app_sprds);
...
int main(int argc, char const *argv[])
{
    ...
    std::vector<apply_spreads> app_sprds;
    populate_spread_true_false_for_records(ed, sprds, app_sprds);

    // 4. Comput Spread Amounts / print recap of data and spreads
    compute_spread_amounts(ed, sprds, app_sprds);
    print_estim_data_w_spreads(ed, sprds, app_sprds);
...
void populate_spread_true_false_for_records(std::vector<estim_data>& ed, std::vector<spread_costs_items>& sprds, std::vector<apply_spreads> &app_sprds) {
    for (auto& e : ed) {
        struct apply_spreads app;
        app.row_id = e.row_id;
        app.apply_TF.resize(sprds.size(), false);

        for (std::vector<struct spread_costs_items>::size_type i = 0; i < sprds.size(); ++i) {
            auto& spr = sprds[i];
...
void compute_spread_amounts(std::vector<estim_data>& ed, std::vector<spread_costs_items>& sprds, std::vector<apply_spreads> &app_sprds) {
    ...
    for (int i=0; i<sprds.size(); i++) {
        ...
        //Loop thru each record in the estimate data 'ed'
        for (int j = 0; j<ed.size(); j++) {
            if(app_sprds[j].apply_TF[i]) {
                 sprds[i].spread_basis_aggregate += ed[j].cost;
            }
        }
    }
    
    // Now calculate the spread ammounts now that the aggregates are available from above
    //Loop thru each spread type and calculate the spread amount for each record
    for (int i=0; i<sprds.size(); i++) {

        //Loop thru each record in the estimate data 'ed'
        for (int j = 0; j<ed.size(); j++) {
    
        // resize the app_sprds vector to match the size of the other sprds vector
            app_sprds[j].sprd_amt.resize(sprds.size(), 0.0);  // ensure that the sprd_amt fields are allocated as same size as sprds.zize()

            if(app_sprds[j].apply_TF[i]) {
                if(sprds[i].spread_basis_aggregate > 0) {
                    app_sprds[j].sprd_amt[i] = sprds[i].amount_to_spread * ed[j].cost / sprds[i].spread_basis_aggregate;
                } else {
                    app_sprds[j].sprd_amt[i] = 0;
                }
            }
        }
    }
...
void print_estim_data_w_spreads(std::vector<estim_data>& ed, std::vector<spread_costs_items>& sprds, std::vector<apply_spreads> app_sprds) {
   ...
   // print titles 
    ...
    const int j_max = app_sprds[0].apply_TF.size();
   ...
   // print elements of the vector
    ...
    for (const auto& e : ed) {
        std::cout   << std::left
            ...
            for (int j=0; j<j_max; j++) {
                std::cout << std::setw(8) << std::boolalpha << app_sprds[i].apply_TF[j];
            }
            for (int j=0; j<j_max; j++) {
                std::cout << std::fixed << std::setprecision(2) << std::setw(10) <<  app_sprds[i].sprd_amt[j];
            }
            i++;
        std::cout << "\n";
    }
