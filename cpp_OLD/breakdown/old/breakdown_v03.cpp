...
struct spread_items {
    ...
    double amt_to_spr_m;    // Amount to Spread Material
    double amt_to_spr_s;
    double amt_to_spr_e;
    double spr_l_agr;
    double spr_m_agr;
...
//7. Write Binary Output File for Excel Import
    ...
    /*7.2*/ void writeString(std::ofstream &file, std::string &str);
    /*7.3*/ void writeDbl(std::ofstream& file, double & num);
    /*7.4*/ void writeInt(std::ofstream& file, int& num);
    /*7.5*/ void writeLong(std::ofstream& file, long& num);
    /*7.6*/ void write_unpivot_data(std::ofstream& outFile, std::vector<unpivot_area_data>& upv, std::vector<spread_items>& sd);
    /*7.7*/ void write_sd(std::ofstream& outFile, std::vector<spread_items>& sd);
    /*7.8*/ void write_final_data(std::ofstream& outFile, std::vector<aggregated_data>& fd);
...
//7. Write Binary Output File for Excel Import
    ...
    /*7.6*/ void write_unpivot_data(std::ofstream& outFile, std::vector<unpivot_area_data>& upv, std::vector<spread_items>& sd) {
        ...
        //Write the Headers
        std::vector<std::string> titles = { "areaNo", "rowNo" ,"brkd_ref" ,"other_mh" ,"area_qty" ,"mh_tot" ,"labor" ,"matl" ,"sub" ,"eq" ,"total" ,"type" ,"div" ,"discp" ,"lab_type" ,"atl_btl" };
        for (auto& s : titles) {
        ...
        for (auto& up : upv) {
            ...
            writeString(outFile, up.brkd_ref );
            writeDbl(outFile, up.other_mh);
            writeDbl(outFile, up.area_qty);
            writeDbl(outFile, up.mh_tot);
            writeDbl(outFile, up.labor);
            writeDbl(outFile, up.matl);
            writeDbl(outFile, up.sub);
            writeDbl(outFile, up.eq);
            writeDbl(outFile, up.total);
            writeString(outFile, up.type);
            writeString(outFile, up.div);
            writeString(outFile, up.discp);
            writeString(outFile, up.lab_type);
            writeString(outFile, up.atl_btl);
